...
function autenticar(email, senha) {// login
    ...
    var instrucaoSql = `
        SELECT idUsuario, nome, email, fkUnidade as unidadeID FROM usuario WHERE email = '${email}' AND senha = '${senha}';//principal
    `;
...
function cadastrar(nome, sobrenome, email, tipo, senha, fkUnidade) {
    console.log("ACESSEI O USUARIO MODEL \n \n\t\t >> Se aqui der erro de 'Error: connect ECONNREFUSED',\n \t\t >> verifique suas credenciais de acesso ao banco\n \t\t >> e se o servidor de seu BD está rodando corretamente. \n\n function cadastrar():", nome, sobrenome, email, tipo, senha, fkUnidade);
 ...
 //O que importa é a ordem dos campos no comando, não a ordem física da tabela no banco.
    ...
    var instrucaoSql = `
        INSERT INTO usuario (nome, sobrenome, email, tipo, senha, fkUnidade) VALUES ('${nome}','${sobrenome}', '${email}', '${tipo}','${senha}', '${fkUnidade}');
    `;
    console.log("Executando a instrução SQL: \n" + instrucaoSql);
    return database.executar(instrucaoSql);
}
